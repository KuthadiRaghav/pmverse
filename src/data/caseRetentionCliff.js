...
const CASE_MIXPANEL = {
  events: ['App Open', 'Item Added to Cart', 'Checkout Started', 'Checkout Completed', 'Email Opened'],
  properties: ['Platform'],
  query: (type, params) => {
    // Mocks an analytics engine for the case data
    const { events, breakdown, dateRange } = params;
    ...
    if (type === 'funnel') {
      ...
      const steps = events.map(e => ({ name: e, desktop: 0, mobile: 0 }));
      
      events.forEach((ev, i) => {
        let mobileDrop = 0.9;
        let desktopDrop = 0.9;
        
        if (ev === 'App Open') { mobileDrop = 1.0; desktopDrop = 1.0; }
        if (ev === 'Item Added to Cart') { mobileDrop = 0.6; desktopDrop = 0.6; }
        if (ev === 'Checkout Started') { mobileDrop = 0.5; desktopDrop = 0.5; }
        if (ev === 'Checkout Completed') {
          // THE CLINCHER: Mobile completion drops dramatically if we include recent weeks (post v2.4)
          // 70% before, 50% after.
          desktopDrop = 0.74;
          mobileDrop = dateRange === 'Last 4 Weeks' ? 0.49 : 0.62; // blended if longer
        }
        
        mobileVol = Math.floor(mobileVol * mobileDrop);
        desktopVol = Math.floor(desktopVol * desktopDrop);
        
        steps[i].mobile = mobileVol;
        steps[i].desktop = desktopVol;
      });
      return steps;
    }
    
    if (type === 'retention') {
      // Generate a classic cohort triangle
      const cohorts = [];
      const isMobile = breakdown === 'Platform: Mobile' || !breakdown;
      const isDesktop = breakdown === 'Platform: Desktop' || !breakdown;
      
      for (let w = 8; w >= 0; w--) {
        const weekLabel = `Week -${w}`;
        let startUsers = 0;
        if (isMobile) startUsers += 1200;
        if (isDesktop) startUsers += 400;
        
        const row = { cohort: weekLabel, size: startUsers, retention: [] };
        // Day 0 is 100%
        row.retention.push(100);
        
        for (let day = 1; day <= 28; day += 7) {
           // Base retention decays over time
           let ret = 100 - (day * 2);
           
           // If it's week -8 or newer, mobile retention collapses at day 28 (week 4)
           if (w <= 8 && day === 28 && isMobile) {
             ret = 22; // The cliff
           } else if (day === 28 && (isDesktop || (!isMobile && isDesktop))) {
             ret = 36;
           } else if (day === 28) {
             ret = 38; // Normal mobile pre-cliff
           }
           
           row.retention.push(ret);
        }
        cohorts.push(row);
      }
      return cohorts;
    ...
    if (type === 'insights') {
      ...
      for (let w = -8; w <= 0; w++) {
        ...
        events.forEach(ev => {
          let val = 1000;
          if (ev === 'App Open') val = 4000;
          if (ev === 'Item Added to Cart') val = 2400;
          if (ev === 'Checkout Started') val = 1200;
          if (ev === 'Checkout Completed') {
             val = w < -8 ? 850 : 600; 
          }
          if (ev === 'Email Opened') val = 3000;
          // Add some jitter for realism
          row[ev] = val + Math.floor((Math.random() * 0.1 - 0.05) * val);
        });
        data.push(row);
      }
      return data;
...
const CASE_SHEETS = {
  ...
  mission: {
    instructionHtml: "Leadership is nervous the fix won't fully recover retention. Change <b>retention-if-fixed (cell B9)</b> to <b>30%</b> and check whether $30K/month is still at stake.",
    evaluate: (readCell) => {
      ...
      if (b9 === null || Number.isNaN(b9)) {
        return { tone: 'info', msg: "Couldn't read the model automatically. Manually: at 30% recovery, revenue-if-fixed = 5200 × 0.30 × 42 × 1.8 = $117,936, so at-stake = $31,450 — still above $30K. The fix clears the bar." };
      }
      if (Math.abs(b9 - 0.30) > 0.005) {
        return { tone: 'warn', msg: `Retention-if-fixed (B9) is still ${(b9 * 100).toFixed(0)}%. Set it to 30% (0.30) to model the conservative case, then check again.` };
      }
      if (b14 !== null && b14 > 30000) {
        return { tone: 'good', msg: `✓ Correct. At 30% recovery, revenue-at-stake recomputes to $${Math.round(b14).toLocaleString()}/month — still above the $30K bar, so the fix is justified even on conservative assumptions.` };
      } else {
        return { tone: 'good', msg: `You updated B9 to 30%. The model recomputes at-stake to ~$31,450/month — above $30K, so the fix holds up even conservatively.` };
      }
    }
  }
};
